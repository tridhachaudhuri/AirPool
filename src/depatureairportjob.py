...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 18 01:33:01 2020
...
def writeheader(filename):
    ...
    header = ['destination_airport_iata','number','callsign', 'live', 'text','type_Arrival','color','diverted',
          ...
          'origin_airport_terminal','origin_airport_baggage','origin_airport_gate',
          'dest_aiportcode_iata','dest_aiportcode_icao','dest_airport_timezone_name','dest_airport_timezone_offset','dest_airport_timezone_abbr','dest_airport_timezone_abbrname',
          'dest_airport_terminal','dest_airport_baggage','dest_airport_gate','dest_airport_latitude',
          ...
          'real_departuredate','real_departuretime','estimated_departuredate','estimated_departuretime']
...
def writeraw(flightdata,airport):
    parentpath=<path-to-save>+str(date.today())+"/departure/"
...
def apidepartedfile(airport_iata,filename):
    
    for airport in airport_iata:
        LAXflights=api.get_airport_departures(airport, limit=100)
        ...
        for i in range(len(LAXflights)):
            ...
            #dest_airport_name=LAXflights[i]['flight']['airport']['destination']['name']
            dest_aiportcode_iata=LAXflights[i]['flight']['airport']['destination']['code']['iata']
            dest_aiportcode_icao=LAXflights[i]['flight']['airport']['destination']['code']['icao']
            
            dest_airport_latitude=LAXflights[i]['flight']['airport']['destination']['position']['latitude']
            dest_airport_longitude=LAXflights[i]['flight']['airport']['destination']['position']['longitude']
            dest_airport_countryname=LAXflights[i]['flight']['airport']['destination']['position']['country']['name']
            dest_airport_countrycode=LAXflights[i]['flight']['airport']['destination']['position']['country']['code']
            dest_airport_city=LAXflights[i]['flight']['airport']['destination']['position']['region']['city']
            ...
            try:
                ...
                real_departuretime=""
            try:
                estimated_arrivaldate=LAXflights[i]['flight']['time']['estimated']['departure_date']
            except:
                estimated_arrivaldate=""
            try:
                estimated_arrivaltime=LAXflights[i]['flight']['time']['estimated']['departure_time']
            except:
